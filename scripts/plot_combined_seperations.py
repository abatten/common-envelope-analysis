...
outfile_name = 'seperations_jstaff_combined_'
yr = 365.25 * 24 * 60 * 60
Rsun = 6.957e10

plt.semilogy(data1[:,0], data1[:,2]/Rsun, 
         'b-', linewidth=2, label=r"$\mathrm{Inner\ Planet}$")
plt.semilogy(data1[:,0], data1[0:,3]/Rsun,
         'r-', linewidth=2, label=r"$\mathrm{Outer\ Planet}$")
plt.semilogy(data2[:,0], data2[:,2]/Rsun, 
         'r--', linewidth=2, label=r"$\mathrm{Outer\ after\ 5.6yr\ merger}$")
plt.semilogy(data3[:,0], data3[:,2]/Rsun, 'r:', 
         linewidth=2, label=r"$\mathrm{Outer\ after\ 4.0yr\ merger}$")
...
plt.tight_layout()
plt.savefig(plot_loc + outfile_name + "_logged.png")
plt.show()
